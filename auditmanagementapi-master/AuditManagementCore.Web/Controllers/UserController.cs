...
namespace AuditManagementCore.Web.Controllers
{
    ...
    public class UserController : VJBaseGenericAPIController<User>
    {
        ...
        public UserController(IMongoGenericRepository<User> api, IEncryption Encryption, IMongoDbSettings mongoDbSettings, IEmailUtility emailUtility, IWebHostEnvironment webHostEnvironment, IGlobalConfiguration config, CommonServices cs) : base(api)
        {
        ...
        public override ActionResult Post([FromBody] User e)
        {
            ...
            e.StakeHolder = false;
            e.UserType = "manual";
            var User = base.Post(e);
            sendMail(e);
            //Activity Log
            _CommonServices.ActivityLog(e.CreatedBy, e.Id, (e.FirstName + " " + e.LastName), "User", "User | Add", "Added User");
            return User;
        }
        public override ActionResult Put([FromBody] User e)
        {
            var tList = _api.GetWithInclude<Role, Company>(x => x.Id == e.Id && x.StakeHolder != true);
            if (tList == null)
            {
                return ResponseError("User does not exists.");
            }
            //populateUser(tList, tValue)
            var user = _api.GetFirst(x => x.Id == e.Id);
            e.CreatedBy = user.CreatedBy;
            e.CreatedOn = user.CreatedOn;
            e.Password = encryption.Encrypt(e.Password);
            e.StakeHolder = false;
            e.UserType = "manual";
            var userUpdated = base.Put(e);
            //Activity Log
            _CommonServices.ActivityLog(e.UpdatedBy, e.Id, (e.FirstName + " " + e.LastName), "User", "User | Edit", "Updated User");
            return userUpdated;
        }
        public override ActionResult Delete(string id, string userid)
        {
            var tList = _api.GetFirst(x => x.Id == id && x.StakeHolder != true);
            if (tList == null)
            {
                return ResponseNotFound();
            }
            var user = base.Delete(id, userid);
            //Activity Log
            _CommonServices.ActivityLog(userid, id, (tList.FirstName + " " + tList.LastName), "User", "User | Delete", "Deleted User");
            return user;
        }
        ...
        {
            var tList = _api.GetWithInclude<Role, Company>(x => x.StakeHolder != true).OrderBy(p => p.FirstName);
                        ...
                        for (int row = 2; row <= rowCount; row++)
                        {
                            try
                            {
                                ...
                                if (exists != null)
                                {
                                    ...
                                    exists.StakeHolder = false;
                                    exists.EmployeeCode = empCode;
                                    _api.Update(exists);
                                }
                                else
                                {
                            ...
                            catch (Exception e)
                            {
                                ExceptionrowCount++;
                                sb.Append(row + ",");
                                _CommonServices.SendExcepToDB(e, "User/ImportExcel()");
                            }
                        }
                    }
                }
        ...
        [HttpPost("sendmailforalluser")]
        public ActionResult SendMailForAllUser()
        {
            //string email= "baldev@silverwebbuzz.com";
            //item.EmailId
            var tList = _api.GetAllWithInclude<Role, Company>();
            try
            {
                foreach (var item in tList)
                {
                    var webRootPath = _IWebHostEnvironment.WebRootPath;
                    var htmlTemplatePath = Path.Combine(webRootPath, "EmailTemplates", "UpdateUser.html");
                    ...
                    EmailModel emailModel = new EmailModel();
                    emailModel.ToEmail = new List<string>() { item.EmailId };
                    emailModel.Subject = "Update on IA Tool";
                    emailModel.MailBody = emailBody.ToString();

                    //_IEmailUtility.SendEmail(emailModel);    just temparary mail
                }
            }
            catch (Exception)
            {
